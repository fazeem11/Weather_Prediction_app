...
def log(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            login(request, user)
            return redirect('predict_weather')  # Replace 'home' with your target view name
        else:
...
def predict_weather(request):
    if request.method == "POST":
        ...
        # Additional rule for Drizzle if the model fails to classify it
        if predicted_label not in ["Drizzle", "Rain", "Sun"]:
            # Example fallback condition for Drizzle
            if 30 <= precipitation <= 70 and 20 <= wind <= 35:
                predicted_label = "Drizzle"
            elif precipitation > 70:
                predicted_label = "Rain"
            elif precipitation < 30 and temp_max > 20:
                predicted_label = "Sun"
